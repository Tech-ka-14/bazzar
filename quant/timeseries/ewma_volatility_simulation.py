...
def simulate_ewma_returns(
    z_matrix: np.ndarray, initial_vol: float, initial_return: float, lambda_param: float
) -> np.ndarray:
    """
    ...
    for t in range(h_days):
        ...
        current_variance = (1 - lambda_param) * (
            previous_return**2
        ) + lambda_param * current_variance
