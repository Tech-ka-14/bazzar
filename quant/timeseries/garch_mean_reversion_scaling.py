...
def compare_garch_vs_sqrt_scaling(
    current_vol: float, long_run_vol: float, alpha_garch: float, beta_garch: float, h_days: int
):
    """
...
print("Scenario 1: High Volatility (Current > Long Run)")
print(f"Naive Sqrt Volatility: {naive_vol_high:.2%}")
print(
    f"True GARCH Volatility: {true_vol_high:.2%} -> Naive rule OVERESTIMATED risk by ignoring downward mean reversion.\n"
)

print("Scenario 2: Low Volatility (Current < Long Run)")
print(f"Naive Sqrt Volatility: {naive_vol_low:.2%}")
print(
    f"True GARCH Volatility: {true_vol_low:.2%} -> Naive rule UNDERESTIMATED risk by ignoring upward mean reversion."
)
