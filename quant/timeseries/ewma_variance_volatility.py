...
def calculate_ewma_variance(
    returns: np.ndarray, lambda_param: float, initial_variance: float = None
) -> np.ndarray:
    """
...
print(
    f"Current Estimated Annual EWMA Volatility (lambda={lambda_smooth}): {ewma_volatilities[-1]:.2%}"
)
