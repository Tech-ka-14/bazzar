...
class FundamentalAnalyzer:
    ...
    def cash_flow_from_assets(
        ocf: float, net_capital_spending: float, change_in_nwc: float
    ) -> float:
    ...
    @classmethod
    def quick_ratio(
        cls, current_assets: float, inventory: float, current_liabilities: float
    ) -> float:
    ...
    @staticmethod
    def dupont_identity(
        profit_margin: float, total_asset_turnover: float, equity_multiplier: float
    ) -> float:
    ...
    def enterprise_value(
        market_cap: float, market_value_interest_bearing_debt: float, cash: float
    ) -> float:
    ...
    def present_value_growing_perpetuity(
        cls, cash_flow_period_1: float, discount_rate: float, growth_rate: float
    ) -> float:
    ...
    def present_value_growing_annuity(
        cls, cash_flow_1: float, discount_rate: float, growth_rate: float, periods: int
    ) -> float:
        if discount_rate == growth_rate:
            return cash_flow_1 * periods / (1 + discount_rate)
        pv_factor = cls.safe_div(
            1 - ((1 + growth_rate) / (1 + discount_rate)) ** periods, discount_rate - growth_rate
        )
    ...
    @classmethod
    def stock_value_constant_growth(
        cls, current_dividend: float, growth_rate: float, required_return: float
    ) -> float:
    ...
    def expected_return_capm(
        risk_free_rate: float, beta: float, expected_market_return: float
    ) -> float:
    ...
    @staticmethod
    def mm_proposition_2_no_taxes(
        r0: float, cost_of_debt: float, debt_val: float, equity_val: float
    ) -> float:
    ...
    @staticmethod
    def put_call_parity_put_value(
        call_price: float, S: float, E: float, t: float, R: float
    ) -> float:
        """P = C + PV(E) - S"""
        try:
            pv_E = E * np.exp(-R * t)
    ...
    @classmethod
    def absolute_purchasing_power_parity(
        cls, spot_rate: float, foreign_price: float, domestic_price: float
    ) -> float:
        """Returns inferred spot rate based on absolute PPP"""
        return cls.safe_div(foreign_price, domestic_price)

    @classmethod
    def relative_purchasing_power_parity(
        cls, current_spot: float, domestic_inflation: float, foreign_inflation: float, periods: int
    ) -> float:
        """Expected future spot rate under Relative PPP"""
        try:
            return float(
                current_spot * ((1 + foreign_inflation) / (1 + domestic_inflation)) ** periods
            )
        except Exception:
            return np.nan
    ...
    def process_financial_dataset(
        ...
        if "Current Assets" in df.columns and "Current Liabilities" in df.columns:
            df["Current Ratio"] = safe_apply(
                self.current_ratio, "Current Assets", "Current Liabilities"
            )

            if "Inventory" in df.columns:
                df["Quick Ratio"] = safe_apply(
                    self.quick_ratio, "Current Assets", "Inventory", "Current Liabilities"
                )

        if "Total Assets" in df.columns and "Total Equity" in df.columns:
            df["Equity Multiplier"] = safe_apply(
                self.equity_multiplier, "Total Assets", "Total Equity"
            )
            df["Total Debt Ratio"] = safe_apply(
                self.total_debt_ratio, "Total Assets", "Total Equity"
            )
...
if __name__ == "__main__":
    ...
    print(
        results[
            ["Company", "Current Ratio", "Quick Ratio", "ROE", "PE Ratio", "Times Interest Earned"]
        ]
    )
    ...
    print(
        f"Black-Scholes Call Option: ${bs_call:.2f}"
        if not pd.isna(bs_call)
        else "BS Call: Error/NaN"
    )

    growth_rate = analyzer.sustainable_growth_rate(roe=0.264, plowback_ratio=0.666)
    print(
        f"Sustainable Growth Rate: {growth_rate:.2%}"
        if not pd.isna(growth_rate)
        else "Growth Rate: Error/NaN"
    )

    cash_cycle = analyzer.cash_cycle(operating_cycle=90, accounts_payable_period=30)
    print(
        f"Cash Cycle (Days): {cash_cycle:.1f}"
        if not pd.isna(cash_cycle)
        else "Cash Cycle: Error/NaN"
    )
