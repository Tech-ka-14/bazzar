...
def calculate_commodity_position_vector(
    contracts: list, prices: list, multiplier: float
) -> np.ndarray:
    """
...
def calculate_commodity_desk_var(
    theta: np.ndarray, omega_1: np.ndarray, horizon_days: int = 10, alpha: float = 0.01
):
    """
...
def calculate_desk_marginal_var(
    theta_total: np.ndarray,
    omega_total_1d: np.ndarray,
    split_index: int,
    horizon_days: int = 10,
    alpha: float = 0.01,
):
...
theta_silver = calculate_commodity_position_vector(
    silver_contracts, silver_prices, silver_multiplier
)
...
omega_total_1d = np.block([[omega_gas_1d, omega_cross], [omega_cross.T, omega_sil_1d]])
...
print(
    f"Marginal Contribution - Silver: ${silver_marginal:,.0f} ({silver_marginal / total_var:.1%})"
)
