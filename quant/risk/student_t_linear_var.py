...
def calculate_student_t_var(
    mu: float, sigma: float, df: float, alpha: float = 0.01, h_days: int = 1
):
    """
    ...
    if df <= 2:
        raise ValueError(
            "Degrees of freedom (nu) must be greater than 2 to have a defined variance."
        )
