...
def simulate_mixture_scenario_var(
    probs: list, means: list, stds: list, n_simulations: int, alpha: float
) -> tuple:
    """
    ...
    if not (len(means) == len(stds) == n_components):
        raise ValueError("probs, means and stds must have equal length")
