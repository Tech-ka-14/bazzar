        ...
        <>
          ...
          <ChartImage
            src={chartUrl(
              'stocks',
              selection.symbol,
              selection.exchange ? { exchange: selection.exchange } : {}
            )}
    ...
    <section className={theme.card}>
      ...
      {info ? (
        <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2 text-sm">
          ...
          <div className="flex justify-between">
            <dt className="text-gray-400">Electron</dt>
            <dd>{info.electron}</dd>
          </div>
          <div className="flex justify-between">
          ...
          <div className="flex justify-between">
            <dt className="text-gray-400">Repository payload</dt>
            <dd>
              {info.repository?.present ? `${info.repository.fileCount} files` : 'not installed'}
            </dd>
          </div>
          <div className="flex justify-between">
            <dt className="text-gray-400">Payload path</dt>
            <dd className="truncate ml-4" title={info.repository?.path}>
        ...
        <p className="text-gray-500 text-sm">
          Running in browser mode (no Electron preload API detected). App and repository payload
          details are available when launched via the Bazzar Terminal desktop shell.
        </p>
      )}
    </section>
  );
